...
from userbot.helpers.misc import HAPP, XCB
from userbot.helpers.tools import get_arg
...
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Update Deploy BayUserbot Sedang Dalam Proses...`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit(
            "`BayUserbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
...
add_command_help(
    "update",
    [
        [f"{cmd}update", "Untuk melihat list pembaruan terbaru dari BayUserbot."],
